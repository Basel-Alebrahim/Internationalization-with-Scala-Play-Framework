...
class MySupportController @Inject()(component: ControllerComponents,
  ...
  def homePageInJapanese = Action {
    Redirect("/").withLang(Lang("jp")) // set japanese language in the play's language cookie for future requests
  }

  def homePageInArabic = Action {
    Redirect("/").withLang(Lang("ar")) //  set arabic language in the play's language cookie for future requests
  }
